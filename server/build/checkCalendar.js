...
exports.checkCalendar = void 0;
const googleApis_1 = require("./googleApis");
const checkCalendar = ({ y, m, length }) => __awaiter(void 0, void 0, void 0, function* () {
    const timeMin = `${y}-${m}-01T00:00`;
    const timeMax = `${y}-${m}-${length}T23:59`;
    const busy = yield googleApis_1.freeBusy(timeMin, timeMax);
    const daysInMonth = [];
    ...
    for (let i = 1; i <= length; i++) {
        ...
        let isAvailble = true;
        if (dayT.getDay() === 0 || dayT.getDay() === 6)
            isAvailble = false;
        daysInMonth.push({
            id: i,
            date: day,
    ...
    for (let i = 0; i < busy.length; i++) {
        ...
        if (daysInMonth[dayIdx].availble) {
            ...
            for (let z = 0; z < periodsForMeeting.length; z++) {
                if (!daysInMonth[dayIdx].periodsForMeeting[z].availble)
                    x++;
            }
            if (x === periodsForMeeting.length) {
                daysInMonth[dayIdx].availble = false;
            }
        }
    }
