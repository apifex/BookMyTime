...
export const checkIsAvailble = async (month) => {
    const monthLenth = new Date(Date.UTC(month.y, Number(month.m),0)).getDate()
    
    let timeMin = `${month.y}-${month.m}-01T00:00`
    let timeMax = `${month.y}-${month.m}-${monthLenth}T23:59`
    ...
    for(let i = 1; i <= monthLenth; i++){
        let dayT = new Date(Date.UTC(month.y, Number(month.m)-1, i));
        let day = `${dayT.getFullYear()}-${month.m}-${i>9?i:'0'+i}`
